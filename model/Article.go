...
// 查询文章列表
func GetArticle(title string, pagSize int, pageNum int) ([]Article, int, int) {
	var articleList []Article
	var total int
	if title == "" {
		err = db.Preload("Category").Find(&articleList).Limit(pagSize).Offset((pageNum - 1) * pagSize).Error
	...
	err = db.Preload("Category").Where("title LIKE ?", title+"%").Find(&articleList).Limit(pagSize).Offset((pageNum - 1) * pagSize).Error
